...
electricalPower = Shaft_Torque.*Speed.*0.10472 + Total_Loss;
p1 =       1.025;
p2 =      -1.301;
p3 =      0.8786;
p4 =       3.605;

% X = STATE OF CHARGE, OCV = open circuit voltage
OCV = @(x) p1*x.^3 + p2*x.^2 + p3.*x + p4;

SOC = linspace(0,1,5);
packVoltage = s*OCV(SOC);
[r,c] = size(electricalPower);
...
for i = 1:length(packVoltage)
    voltageDrop = (packVoltage(i)./electricalPower).*packInternalResistance;
